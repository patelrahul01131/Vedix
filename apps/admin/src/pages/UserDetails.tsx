    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }}>
        ...
        <div className="glass-panel" style={{ padding: '24px' }}>
          <h3 style={{ margin: '0 0 16px 0', fontSize: '1.25rem' }}>Missions ({user.missions.length})</h3>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', maxHeight: '400px', overflowY: 'auto' }}>
            {user.missions.map((m: any) => (
              <div key={m.id} style={{ background: 'rgba(0,0,0,0.2)', padding: '12px', borderRadius: '8px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                  ...
                  <span className={m.status === 'Completed' ? 'badge badge-success' : 'badge badge-warning'}>{m.status}</span>
                </div>
            ...
            {user.missions.length === 0 && <span style={{ color: 'var(--text-secondary)' }}>No missions found.</span>}
          </div>
        </div>
