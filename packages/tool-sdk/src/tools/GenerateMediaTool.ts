...
export class GenerateMediaTool extends Tool {
  ...
  schema: ToolSchema = {
    ...
    properties: {
      prompt: {
        type: 'string',
        description: 'The detailed visual description of the media to generate. YOU MUST ACT AS A PROMPT ENGINEER: Expand short user requests into highly detailed, descriptive, and visually rich paragraphs. Focus on lighting, style, medium, subject details, and environment.'
      },
      negative_prompt: {
        type: 'string',
  ...
  async execute(args: any): Promise<any> {
    ...
    try {
      ...
      return {
        status: 'success',
        message: 'Media generated and uploaded to MinIO storage successfully.',
        prompt_id: promptId,
        mock_output: type === 'video' ? `[video](${minioUrl})` : `![Generated Media](${minioUrl})`
      };
    } catch (error: any) {
